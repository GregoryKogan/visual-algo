...
export class DFSSketch {
  ...
  async dfs(pid: number, v: number, parent = -1): Promise<void> {
    ...
    if (parent != -1) {
      ...
      if (ind != -1) this.coloredEdges.splice(ind, 1);
  ...
  renderVertecies(): void {
    for (let i = 0; i < this.n; ++i) {
      ...
      if (i == 0) this.painter.fill("#ff5555");
      else if (this.visited[i]) this.painter.fill("#50fa7b");
      if (i == this.currentV) this.painter.fill("#ff79c6");
  ...
  renderUsedEdges(): void {
    for (let i = 0; i < this.coloredEdges.length; ++i) {
      const from = this.coloredEdges[i].from;
      const to = this.coloredEdges[i].to;
      if (this.coloredEdges[i].color == 1) this.painter.stroke("#ffb86c");
      else if (this.coloredEdges[i].color == 2) this.painter.stroke("#8be9fd");
